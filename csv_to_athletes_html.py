...
<body data-theme="light">
    ...
    // Define HTML for icons
    const srIconHTML = '<i class="fas fa-shoe-prints"></i><i class="fas fa-shoe-prints"></i>'; // Pair of shoes
    const prIconHTML = '<i class="fas fa-stopwatch"></i><i class="fas fa-arrow-up"></i>'; // Timer and up arrow

    // Function to replace "SR" and "PR" text with icons within specific elements
    function replaceTextWithIcons() {
        // Target specific elements (e.g., table cells or elements with a specific class)
        const elements = document.querySelectorAll("table td, .replaceable-text");

        elements.forEach(element => {
            element.childNodes.forEach(node => {
                if (node.nodeType === Node.TEXT_NODE) {
                    // Replace "SR" with shoe icons
                    if (node.textContent.includes("SR")) {
                        node.parentNode.innerHTML = node.textContent.replace(/\bSR\b/g, srIconHTML);
                    }
                    // Replace "PR" with timer and up arrow icons
                    if (node.textContent.includes("PR")) {
                        node.parentNode.innerHTML = node.textContent.replace(/\bPR\b/g, prIconHTML);
                    }
                }
            });
        });
